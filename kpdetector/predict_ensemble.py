...
if __name__ == '__main__':
    ...
    checkpoint = torch.load(root_path + 'checkpoints/trousers_027_posneu_lgtrain.ckpt')  # must before cuda
    net.load_state_dict(checkpoint['state_dict'])
    net = net.cuda()
    net = DataParallel(net)
    net.eval()
    net2 = CascadePyramidNet(config)
    checkpoint = torch.load(root_path + 'checkpoints/trousers_036_lgtrain.ckpt')  # must before cuda
    ...
    print(df.head(5))
    df.to_csv(root_path+'kp_predictions/'+config.clothes+'.csv', index=False)
