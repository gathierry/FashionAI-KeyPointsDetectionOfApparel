...
class GlobalNet(nn.Module):
    ...
    def forward(self, x):
        ...
        p4 = self.upsample1(p5) + self.latlayer2(c4)  #self._upsample_add(p5, self.latlayer2(c4))
        p4 = self.toplayer1(p4)
        p3 = self.upsample2(p4) + self.latlayer3(c3)  #self._upsample_add(p4, self.latlayer3(c3))
        p3 = self.toplayer2(p3)
        p2 = self.upsample3(p3) + self.latlayer4(c2)  #self._upsample_add(p3, self.latlayer4(c2))
...
if __name__ == '__main__':
    ...
    net = CascadePyramidNetV7(config)
