...
class RefineNet(nn.Module):
    def __init__(self, config):
        ...
        self.bottleneck2 = nn.Sequential(Bottleneck(256, 64, 1),
        ...
        self.bottleneck5 = nn.Sequential(Bottleneck(256, 64, 1),
                                         Bottleneck(256, 64, 1),
                                         Bottleneck(256, 64, 1),
                                         nn.ConvTranspose2d(256, 256, kernel_size=2 * 16, stride=16, padding=16 // 2))
        self.output = nn.Sequential(Bottleneck(256*5, 64, 1),
...
def GlobalNet50(config, pretrained=False):
    if pretrained:
        return GlobalNet(config, Bottleneck, [3, 4, 6, 3], torchvision.models.resnet50(pretrained=True))
    return GlobalNet(config, Bottleneck, [3, 4, 6, 3])

def GlobalNet101(config, pretrained=False):
    if pretrained:
        return GlobalNet(config, Bottleneck, [3, 4, 23, 3], torchvision.models.resnet101(pretrained=True))
    return GlobalNet(config, Bottleneck, [3, 4, 23, 3])

def GlobalNet152(config, pretrained=False):
    if pretrained:
        return GlobalNet(config, Bottleneck, [3, 8, 36, 3], torchvision.models.resnet152(pretrained=True))
    return GlobalNet(config, Bottleneck, [3, 8, 36, 3])


### Resolution for all level
...
if __name__ == '__main__':
    ...
    net = CascadePyramidNetV5(config)
