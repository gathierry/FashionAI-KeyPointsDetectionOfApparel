# Implemented by Cadene https://github.com/Cadene/pretrained-models.pytorch
...
class InceptionResNetV2(nn.Module):
    ...
    def features(self, input):
        x = self.conv2d_1a(input)  # downsampled
        x = self.conv2d_2a(x)
        x = self.conv2d_2b(x)  # ch=64
        x = self.maxpool_3a(x)  # downsampled
        x = self.conv2d_3b(x)
        x = self.conv2d_4a(x)  # ch=192
        x = self.maxpool_5a(x)  # downsampled
        x = self.mixed_5b(x)
        x = self.repeat(x)  # ch=320
        x = self.mixed_6a(x)  # downsampled
        x = self.repeat_1(x)  # ch=1088
        x = self.mixed_7a(x)  # downsampled
        x = self.repeat_2(x)
        x = self.block8(x)
        x = self.conv2d_7b(x)  # ch=1536
        return x
