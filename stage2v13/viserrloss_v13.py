...
def vis_err_loss_np(targets, preds, vismap):
    preds[preds<0] = 0
    peak = np.amax(targets)
    pos_ids = (targets > (peak / 10)) & (vismap == 1)
    neg_ids = (targets <= (peak / 10)) & (vismap == 1)
    if np.sum(pos_ids) == 0:
        pos_loss = 0
    else:
        pos_loss = np.mean(np.abs(targets[pos_ids] - preds[pos_ids]))
    if np.sum(neg_ids) == 0:
        neg_loss = 0
    else:
        neg_loss = np.mean(np.abs(targets[neg_ids] - preds[neg_ids]))
    return 0.5 * pos_loss + 0.5 * neg_loss, pos_loss, neg_loss
